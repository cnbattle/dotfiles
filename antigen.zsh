...
# Antigen: A simple plugin manager for zsh
 ...
 #          and Contributors <https://hub.fastgit.org/zsh-users/antigen/contributors>
...
# Usage:
#   -antigen-bundle-short-name "https://hub.fastgit.org/user/repo.git[|*]" "[branch/name]"
# Returns:
...
# Usage:
#  -antigen-get-clone-dir "https://hub.fastgit.org/zsh-users/zsh-syntax-highlighting.git[|feature/branch]"
# Returns:
...
#
# Usage
#     -antigen-parse-branch https://hub.fastgit.org/user/repo.git x.y.z
#
# Returns
#     Branch name
...
-antigen-load () {
  ...
  if [[ -f "${location}" ]]; then
    ...
    if [[ $#list == 0 ]]; then
      ...
      # Support prezto function loading. See https://hub.fastgit.org/zsh-users/antigen/pull/428
      ...
      *)
        ...
        case $index in
          0)
            ...
            # Full url with protocol or ssh github url (hub.fastgit.org:org/repo)
            if [[ "$value" =~ "://" || "$value" =~ ":" ]]; then
              if [[ "$value" =~ [@.][^/:]+[:]?[0-9]*[:/]?(.*)@?$ ]]; then
                url_path=$match[1]
...
antigen-bundles () {
  ...
  local line
  setopt localoptions no_extended_glob # See https://hub.fastgit.org/zsh-users/antigen/issues/456
